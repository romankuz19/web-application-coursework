...
$query = $pdo->query("SELECT * FROM `chrysalis` WHERE `typename` = '$typename'");
...
echo ''.$totalprice.'';
    ...
    echo '<br>'.$orderid.'';
        ...
        $mysql->query("INSERT INTO `orders`(`dollid`, `userid`, `price`,`orderedamount`) VALUES ($dollid,$userid,$totalprice,$orderedamount)");
        
    
        // $bool=0;
        
        $mysql->query("UPDATE `chrysalis` SET `amount`='$leftamount' WHERE `id`=$dollid");
        $mysql->close();
        ...
        header('Location: /butterflydolls.php');
}
else{
