...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  <div class="type-container d-flex justify-content-around">
      ...
      <?php
             require 'configDB.php';
             $query = $pdo->query("SELECT * FROM `chrysalis`");
               
             echo '<ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">';
             $countnames=1;
             while($row = $query->fetch(PDO::FETCH_OBJ)){
            echo '<li><a class="dropdown-item" style="cursor:pointer" id="linkname'.$countnames.'" onclick="setName('.$countnames.')">'.$row->typename.'</a></li>';
            $countnames++;
             }
...
<input type="text" class="form-control" name="dolltype" id="dolltype" placeholder="Введите имя куколки, которую нужно убрать"> <br>


<div class="d-flex justify-content-center">
<button class="btn btn-danger" type="submit" name="upload">Убрать куколку</button> <br>
</div>
</div>
