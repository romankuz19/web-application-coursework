...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
  ...
  <div class="type-container d-flex justify-content-around">
      ...
      <?php
            ...
            echo '<li><a class="dropdown-item" style="cursor:pointer" id="linktype'.$counttypes.'" onclick="setType('.$counttypes.')">'.$row->name.'</a></li>';
            $counttypes++;
             }
             echo '</ul>';
             echo '';
    ...
    <br>
      ...
      <?php
          ...
          if($row4->admin == 1):
        ?>
            <form class="form1 w-50" action="/add1.php" method="post" enctype="multipart/form-data">

        
            
<input type="text" class="form-control" name="cagename" id="parrotname" placeholder="Введите название клетки"> <br>

<input type="text" class="form-control" name="typename1" id="typename" placeholder="Введите вид попугая, подходящий для клетки"> <br>

<input type="number" class="form-control" name="cageamount" placeholder="Введите количество клеток"> <br>

<input type="number" class="form-control" name="cageprice" placeholder="Введите цену клетки"> <br>
...
<button class="btn btn-primary" type="submit" name="upload">Добавить клетку</button> <br>
</div>
